...
UaPlots(CtrlVar,MUA,F,R,PlotUnderMesh=true,FigureTitle="ElementErrorEstimator");
...
% If there are other mesh-refinement criteria being applied, then some elements may have been marked for coarsening.
% Therefore make sure that the elements marked for refinement are also not marked for coarsening:
%
%   ElementsToBeRefined(iR)=true ; 
%   ElementsToBeCoarsened(iR)=false;
%
% And if all other previous mesh refinement options should be discarded, reset as follows;

ElementsToBeRefined=iR ; 
...
PlotMuaMesh(CtrlVar,MUA);  hold on ;  
CtrlVar.MarkElements=true;
PlotMuaMesh(CtrlVar,MUA,ElementsToBeCoarsened,"b")

FindOrCreateFigure("R histogram") ; 
HI=histogram(R) ; title("Histogram of the recovery-based error estimator ($R$)",interpreter="latex") ; 
xlabel("$R$",Interpreter="latex")
ylabel("Percentage of elements",Interpreter="latex")
HI.Normalization="percentage";
hold on ; xline(Rcrit,color="r",LineWidth=2)
%%
end
