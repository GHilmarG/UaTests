function UserVar=DefineOutputs(UserVar,Outs)

[CtrlVar,MUA,time,s,b,h,S,B,ub,vb,ud,vd,dhdt,dsdt,dbdt,C,AGlen,m,n,rho,rhow,g,as,ab,GF,BCs,l]=Outs2Vars(Outs);


persistent FigThickMin FigSpeed FigVol

plots='-ubvb-e-save-V(t)-';
plots='-speed-ThickMin-V(t)-';
...
if ~isempty(strfind(plots,'-V(t)-'))
    
    [TotalIceVolume,ElementIceVolume]=CalcIceVolume(CtrlVar,MUA,h);
    
    if isempty(FigVol) ; FigVol=figure ; else  FigVol=figure(FigVol) ; end
    
    hold on
    plot(time,TotalIceVolume,'o')
...
if ~isempty(strfind(plots,'-ThickMin-'))
    % plotting horizontal velocities
    
    if isempty(FigThickMin)
        
        % make one initial figure showing the situation at start
        figure
        PlotFEmesh(MUA.coordinates,MUA.connectivity,CtrlVar)
        I=h<=CtrlVar.ThickMin;
        
        hold on
        plot(x(I)/CtrlVar.PlotXYscale,y(I)/CtrlVar.PlotXYscale,'or')
        hold off
        
        FigThickMin=figure;
    else
        FigThickMin=figure(FigThickMin);
    end
