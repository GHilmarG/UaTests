...
isReadData=true;
isDefinePolygonArea=false;
isSaveInterpolants=false;
...
if isReadData
    ...
    ocean=ncread(wdir+filenameITS,"ocean");   fprintf("ocean ...")
    ...
    rockRegion=rock(ix,iy); iceRegion=ice(ix,iy);    oceanRegion=ocean(ix,iy); 
...
if isSaveInterpolants
    ...
    Focean=griddedInterpolant({xRegion,yRegion},single(oceanRegion)) ;
    Fice=griddedInterpolant({xRegion,yRegion},single(iceRegion)) ;


    save("RegionalInterpolants","Fs","Fvx","Fvy","FvxError","FvyError","Fas","Frock","Fice","Focean")
