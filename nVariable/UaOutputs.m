...
if strcmpi(CtrlVar.UaOutputsInfostring,'First call')
    return
end
...
if ~isempty(strfind(plots,'-plot-'))
    ...
    GLgeo=[]; xGL=[] ; yGL=[];
    %%
    exxAnalytical=AGlen.*(g*rho.*(1-rho./rhow).*h/4).^n;
    [exxb,eyyb,exyb]=CalcNodalStrainRates(CtrlVar,MUA,ub,vb);
    fig100=figure(100) ;
    fig100.Position=[50 50 figsWidth 3*figHeights];
    
    subplot(4,1,1)
    PlotMeshScalarVariable(CtrlVar,MUA,exxb-exxAnalytical); title(sprintf('exx-exxAnalytical at t=%g',time))
    hold on    
    ...
    subplot(4,1,4)
    PlotMeshScalarVariable(CtrlVar,MUA,exxAnalytical);   title(sprintf('exx Analytical at t=%g',time))
    hold on
    ...
    plot(x/1000,exxb,'.b')
    hold on 
    plot(x/1000,exxAnalytical,'or')
    legend('Numerical','Analytical')
    xlabel('x (km)') ; ylabel('exx (1/yr)')
    text(100,(exxAnalytical(1)+exxAnalytical(end))/2,'n=3')
    text(500,(exxAnalytical(1)+exxAnalytical(end))/2,'n=5')
