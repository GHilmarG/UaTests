...
%% periodic BCs
% BCs.ubFixedNode=[] ;  BCs.ubFixedValue=[];
...
%BC.vbTiedNodeA=[setdiff(nodesu,nodesya);nodesl]; BC.vbTiedNodeB=[setdiff(nodesd,nodesyb);nodesr];



% BCs.hFixedNode=[];  BCs.hFixedValue=[];
...
if ~isempty(BCs.vbFixedNode)
    [BCs.vbFixedNode,ind]=unique(BCs.vbFixedNode);  BCs.vbFixedValue=BCs.vbFixedValue(ind);
end

if ~isempty(BCs.ubFixedNode)
    [BCs.ubFixedNode,ind]=unique(BCs.ubFixedNode);  BCs.ubFixedValue=BCs.ubFixedValue(ind);
end
