...
persistent FuMeas FvMeas FvxErrMeas FvyErrMeas VelocityInterpolants  % keep scattered interpolants for the data in memory.


if isstring(VelocityInterpolants) &&  VelocityInterpolants~=UserVar.Files.VelocityInterpolants

    % This is in case the user has changed the vel. interpolants from last run, in which case the FuMeas, etc, must be re-read
    % anew, even if already in local memory.

    VelocityInterpolants=UserVar.Files.VelocityInterpolants; 
    FuMeas=[]; FvMeas=[]; FvxErrMeas=[]; FvyErrMeas=[] ;  
...
if isempty(FuMeas)
    ...
    fprintf('Loading interpolants for surface velocity data: %-s ',UserVar.Files.VelocityInterpolants)
     
    load(UserVar.Files.VelocityInterpolants,"FuMeas","FvMeas","FvxErrMeas","FvyErrMeas")
    fprintf(' done.\n')
end
...
usErr=double(FvxErrMeas(F.x,F.y));
vsErr=double(FvyErrMeas(F.x,F.y));
Err=sqrt(usErr.*usErr+vsErr.*vsErr);

% UaPlots(CtrlVar,MUA,F,Err,FigureTitle="Error") ; set(gca,'ColorScale','log') ;  colormap(othercolor("Mlightterrain",20) ) ; clim([0.01 10])
...
MissingData=isnan(Meas.us) | isnan(Meas.vs) | isnan(Err) ;
Meas.us(MissingData)=0 ;  Meas.vs(MissingData)=0 ; Err(MissingData)=1e10;

% For some odd reason, the error is zero over large parts of Greenland...?!  I therefore is forced to set some minimum error
ErrMin=1;
