...
%error('fsad')    
    ...
    dlmwrite([CtrlVar.geo,'-GL.dat'],[time GLx GLstd],'-append')
    ...
    if ~isempty(strfind(plots,'-h(x)-'))
     ...
     % u(x) 
    if ~isempty(strfind(plots,'-u(x)-'))
        %  lower surface
        figure(140)
        hold off
        x=coordinates(:,1) ;
        [xsorted,I]=sort(x);
        plot(x(I)/CtrlVar.PlotXYscale,u(I),'.')
        xlabel('x') ; ylabel('u') ;
        hold on
        title(sprintf('u(x) at t=%#5.1g ',time))
        hold off
    ...
    if ~isempty(strfind(plots,'-GF-'))
      ...
      if ~isempty(strfind(plots,'-u-'))
        %  lower surface
        figure(91)
        hold off
        PlotNodalBasedQuantities(connectivity,coordinates,u,CtrlVar);
        xlabel('xps (km)') ; ylabel('yps (km)') ;
        colorbar ; title(colorbar,'(m)')
        hold on
      ...
      if ~isempty(strfind(plots,'-v-'))
          %  lower surface
          figure(92)
          hold off
          PlotNodalBasedQuantities(connectivity,coordinates,v,CtrlVar);
          xlabel('xps (km)') ; ylabel('yps (km)') ;
          colorbar ; title(colorbar,'(m)')
          hold on
    ...
    if ~isempty(strfind(plots,'-sbB-'))
        ...
        axis equal ; tt=daspect ; daspect([mean(tt(1)+tt(2)) mean(tt(1)+tt(2)) 1e5*tt(3)/CtrlVar.PlotXYscale]); axis tight
        hold off
    end
