...
%     dvb(~F1.LSFMask.NodesIn)=NaN;
    ...
    dub(VAF2node<eps)=NaN;
    dvb(VAF2node<eps)=NaN;

    CtrlVar.VelColorMap=jet(100) ;
    cbar=QuiverColorGHG(F1.x,F1.y,dub,dvb,CtrlVar) ;
    title(cbar,"(m/a)")
    title(sprintf("%s \n velocities difference at t=%3.1f",Compare,CtrlVar.time),Interpreter="latex")
    ...
    if AskForInput
        prompt = " Y/N/C [Y]: ";
        txt = input("Ret to continue, N for break, C for continous: ","s");
        if txt=="C" || txt=="c"
            AskForInput=false;
        elseif txt=="N" || txt=="n"
            break
