...
T =table(threads,seq_BU2IH,seq_C2300,SPMD_BU2IH,SPMD_C2300);  

hold off; figure(10) ; hold off
plot(T.threads,T.SPMD_BU2IH,'or-',DisplayName="Machine with 16 cores")
hold on
plot(T.threads,T.SPMD_C2300,'ob-',DisplayName="Machine with 32 cores")
leg=legend;
xlabel("\# element partictions",Interpreter="latex")
ylabel("speedup with respect to 1 element partition",Interpreter="latex");
...
SPMD_BU2IH=[     1                1.3                 1.6          4.2       4.5       4.8         4.3       ]';
SPMD_C2300  =[   1               1.1                 1.2           4.7       4.7       5.2        4.6       ]'; 

% C2300   : 32 Cores, 64 logical
% BU2IHIR : 16 cores, 32 logical
...
-parfeval performance seems to become worse with number of uv iteratations, fetching the data from the workers takes longer and longer event though the arrays sizes are not changing.
